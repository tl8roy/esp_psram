...
impl Identification {
    ...
    pub fn from_bytes<SPI: Transfer<u8>, CS: OutputPin>(buf: &[u8]) -> Result<Self,Error<SPI, CS>> {
...
impl<SPI: Transfer<u8>, CS: OutputPin> PSRAM<SPI, CS> {
    ...
    pub fn init(spi: SPI, cs: CS, freq: Freq, burst_length: BurstLength) -> Result<Self, Error<SPI, CS>> {
        let mut this = Self { spi, cs, freq, burst_length };
...
impl<SPI: Transfer<u8>, CS: OutputPin> SingleWrite<u8,u32> for PSRAM<SPI, CS> 
{
    ...
    fn try_write(&mut self, address: Address<u32>, word: u8) -> nb::Result<(), Self::Error> {
        self.try_write_slice(address, &[word])
    }
}
